<template>
  <QLayout view="lHh LpR lFf" class="bg-white" :style="layoutStyles">
    ...
    <BsLayoutDrawer
      ...
      :mini="isTabsMultiple"
    ></BsLayoutDrawer>
    <BsLayoutHeader @vnode-mounted="headerMounted = true"></BsLayoutHeader>
    <!-- 
        ===================
        -----MENU-TABS-----
        ===================
...
export default defineComponent({
  ...
  provide() {
    const provideStatic = this.provideStatic(["tabs"]);
  ...
  computed: {
    ...
    activeTabSlots() {
      return Object.keys(this.$slots).filter((slot) =>
        this.tabSlotNames.includes(slot)
      );
    },
    ...
    qLayoutMounted() {
      console.log("DEBUG FROM LAYOUT MOUNTED");
      console.log(this.drawerMounted && this.headerMounted);
